...
def read_engine(path, engine_name=None):
    ...
    # Note: Utilization percentage ('val') is now added from tegrastats data
    # in EngineService.get_status() method, not here
    return engine
    ...
    def get_status(self, tegrastats_data=None):
        ...
        for engine in self.engines_path:
            ...
            for local_path in self.engines_path[engine]:
                ...
                # Read engine frequency data from sysfs
                engine_data = read_engine(local_path, engine_name=name_engine)

                # Merge utilization percentage from tegrastats if available
                if tegrastats_data and name_engine in tegrastats_data:
                    tegra_engine_data = tegrastats_data[name_engine]
                    # tegra_engine_data is {'val': percentage, 'frq': frequency} or {'val': percentage}
                    if isinstance(tegra_engine_data, dict) and 'val' in tegra_engine_data:
                        engine_data['val'] = tegra_engine_data['val']
                    elif isinstance(tegra_engine_data, (int, float)):
                        # Sometimes it's just a number
                        engine_data['val'] = int(tegra_engine_data)

                status[engine][name_engine] = engine_data
        return status
# EOF
